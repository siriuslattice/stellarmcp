...
export function registerOperationTools(server: McpServer, horizon: HorizonClient, _config: Config) {
  server.tool(
    "getOperations",
    "Get recent operations for a Stellar account",
    {
      accountId: z.string().describe("Stellar account ID (G... address)"),
      limit: z.number().min(1).max(50).default(10).describe("Number of operations to return (1-50)"),
    },
